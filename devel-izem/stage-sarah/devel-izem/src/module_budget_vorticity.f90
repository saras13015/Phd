...
!> \brief Calculate the volume vorticity destruction or destruction.
!!
!!
!> \author
!! Modeling, Simulation and Data Analysis (MSDA)\n
...
!! Benguerir, Morocco
  ...
  subroutine vorticity_volume_viscosity_production ( dx_i , dy_i , dz_i , v , kappa , vol_vort_prod )
    ...
    real (dp) , dimension (:) , allocatable , intent (in)                :: dz_i          !< inverted dz array
    real (dp) , dimension (:,:,:,:) , allocatable , intent (in)          :: v             !< conserved variables array
    real (dp) , dimension (:,:,:) , allocatable , intent (in)            :: kappa         !< Reynolds stress tensor
    real (dp) , dimension (:,:,:) , allocatable , intent (inout)         :: vol_vort_prod !< vorticity viscous term


    integer (ip)                                        :: ok , i , j , k
    real (dp)                                           :: rho_i
    real (dp) , dimension (:,:,:) , allocatable         :: ux , vy 
    real (dp) , dimension (:,:,:) , allocatable         :: dxvarx , dyvary 
    real (dp) , dimension (:,:,:) , allocatable         :: wrk1 , dxwrk1 , dywrk1
    real (dp) , dimension (:,:,:) , allocatable         :: rho , dxvarrho  , dyvarrho
    ...
    allocate ( ux        (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)    , &
               ...
               dxvarx    (sx:ex,sy:ey,sz:ez)                      , &
               dyvary    (sx:ex,sy:ey,sz:ez)                      , &
               dxwrk1    (sx:ex,sy:ey,sz:ez)                      , &
               dywrk1    (sx:ex,sy:ey,sz:ez)                      , &
               dxvarrho  (sx:ex,sy:ey,sz:ez)                      , &
               dyvarrho  (sx:ex,sy:ey,sz:ez)                      , &
               stat = ok )
               
    if ( ok > 0 ) stop 'error allocate vorticity_volume_viscosity_production'
    
       do k = sz,ez
          do j = sy,ey
             do i = sx,ex
                rho (i,j,k) = v (i,j,k,1)
                ux (i,j,k)  = v (i,j,k,2) / rho (i,j,k)
                vy (i,j,k)  = v (i,j,k,3) / rho (i,j,k)
             end do
          end do
       end do

       call comm_one (ux) ; call comm_one (vy) 
       call dx ( dx_i , ux , dxvarx ) ; call dy ( dy_i , vy , dyvary )
       ...
       do k = sz,ez
          do j = sy,ey
             do i = sx,ex
                wrk1(i,j,k) = kappa(i,j,k) * ( dxvarx(i,j,k) + dyvary(i,j,k) )
             end do
          end do
       end do
       ...
       call dx ( dx_i , wrk1 , dxwrk1 ) ; call dy ( dy_i , wrk1 , dywrk1 ) 

       do k = sz,ez
          do j = sy,ey
             do i = sx,ex
                vol_vort_prod (i,j,k) = dxvarrho (i,j,k) * dywrk1 (i,j,k) - &
                                        dyvarrho (i,j,k) * dxwrk1 (i,j,k)
                vol_vort_prod (i,j,k) = vol_vort_prod (i,j,k) / ( rho (i,j,k) * rho (i,j,k) )
                                
             end do
          end do
       end do
 
       deallocate ( ux , vy , dxvarx , dyvary , &
                    wrk1 , dxwrk1 , dywrk1    , &
                    rho , dxvarrho  , dyvarrho )

  end subroutine vorticity_volume_viscosity_production
  
  
!> \brief Calculate the shear vorticity destruction or destruction.
!!
!!
!> \author
!! Modeling, Simulation and Data Analysis (MSDA)\n
...
!! Benguerir, Morocco
  ...
  subroutine vorticity_shear_viscosity_production ( dx_i , dy_i , dz_i , v , mu , shear_vort_prod )
    ...
    real (dp) , dimension (:) , allocatable , intent (in)                :: dz_i            !< inverted dz array
    real (dp) , dimension (:,:,:,:) , allocatable , intent (in)          :: v               !< conserved variables array
    real (dp) , dimension (:,:,:) , allocatable , intent (in)            :: mu              !< Reynolds stress tensor
    real (dp) , dimension (:,:,:) , allocatable , intent (inout)         :: shear_vort_prod !< vorticity viscous term
    ...
    real (dp) , dimension (:,:,:) , allocatable         :: rho , aux , dx_aux , dy_aux        
    real (dp) , dimension (:,:,:) , allocatable         :: dxwrk11 , dywrk21 , dxwrk12 , dywrk22
    real (dp) , dimension (:,:,:) , allocatable         :: dx_ux , dy_ux , dx_vy , dy_vy
    ...
    allocate ( ux        (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)                    , &
               ...
               aux       (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)                    , &
               dx_aux    (sx:ex,sy:ey,sz:ez)                                      , &
               dy_aux    (sx:ex,sy:ey,sz:ez)                                      , &
               dxwrk11   (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)                    , &
               dywrk22   (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)                    , &
               dxwrk12   (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)                    , &
               dywrk21   (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)                    , &
               wrk       (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng, ndimmax , ndimmax) , &
               stat = ok )
               
    if ( ok > 0 ) call abort_mpi ('vorticity_shear_viscosity_production')
    ...
    call dx ( dx_i , vy , dx_vy ) ; call dy ( dy_i , vy , dy_vy )
       ...
       do k = sz,ez
          do j = sy,ey
             do i = sx,ex
                wrk(i,j,k,1,1) = mu (i,j,k) * ( dx_ux(i,j,k) + dx_ux(i,j,k) - &
                                 2.0_dp / 3.0_dp  * ( dx_ux(i,j,k) + dy_vy(i,j,k) ) )
                wrk(i,j,k,2,2) = mu (i,j,k) * ( dy_vy(i,j,k) + dy_vy(i,j,k) - &
                                 2.0_dp / 3.0_dp  * ( dx_ux(i,j,k) + dy_vy(i,j,k) ) )                
                wrk(i,j,k,1,2) = mu (i,j,k) * ( dy_ux(i,j,k) + dx_vy(i,j,k) )
                wrk(i,j,k,2,1) = mu (i,j,k) * ( dx_vy(i,j,k) + dy_ux(i,j,k) )
             end do
          end do
       end do
       ...
       do k = sz,ez
          do j = sy,ey
             do i = sx,ex
                aux(i,j,k) = wrk(i,j,k,1,1)         
             end do
          end do
       end do          
       
       call comm_one (aux) ; call dx ( dx_i , aux , dxwrk11 )
       
       do k = sz,ez
          do j = sy,ey
             do i = sx,ex
                aux(i,j,k) = wrk(i,j,k,2,1)         
             end do
          end do
       end do          
       
       call comm_one (aux) ; call dy ( dy_i , aux , dywrk21 ) 
       
       do k = sz,ez
          do j = sy,ey
             do i = sx,ex
                aux(i,j,k) = wrk(i,j,k,1,2)         
             end do
          end do
       end do          
       
       call comm_one (aux) ; call dx ( dx_i , aux , dxwrk12 )        
             
       do k = sz,ez
          do j = sy,ey
             do i = sx,ex
                aux(i,j,k) = wrk(i,j,k,2,2)         
             end do
          end do
       end do          
       
       call comm_one (aux) ; call dy ( dy_i , aux , dywrk22 )  
       ...
       do k = sz,ez
          do j = sy,ey
             do i = sx,ex
                rho_i      = 1.0_dp / v (i,j,k,1)
                aux(i,j,k) =  ( dxwrk11(i,j,k) +  dywrk21(i,j,k) ) * rho_i     
             end do
          end do
       end do
       
       call comm_one (aux) ; call dx ( dx_i , aux , dx_aux )
       
       do k = sz,ez
          do j = sy,ey
             do i = sx,ex
                rho_i      = 1.0_dp / v (i,j,k,1)
                aux(i,j,k) =  ( dywrk22(i,j,k) +  dxwrk12(i,j,k) ) * rho_i      
             end do
          end do
       end do
       
       call comm_one (aux) ; call dy ( dy_i , aux , dy_aux )
       
       do k = sz,ez
          do j = sy,ey
             do i = sx,ex
                shear_vort_prod(i,j,k) =  dx_aux(i,j,k) - dy_aux(i,j,k)    
             end do
          end do
       end do       
       
    deallocate ( ux , vy , dx_ux , dy_ux , dx_vy , dy_vy ,  &
                 aux , dx_aux , dy_aux , wrk , &
                 dxwrk11 , dywrk21 , dxwrk12 , dywrk22 ) 
    
  end subroutine vorticity_shear_viscosity_production
