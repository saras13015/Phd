...
!! Benguerir, Morocco
  ...
  type sim_type
     ...
     integer (ip)                                         :: correct_endfile !< corrected endfile if skypfile differs from unity
...
!! Benguerir, Morocco
  ...
  type inst_type
     ...
     real (dp) , dimension (:,:,:) , allocatable     :: xiv2
...
!! Benguerir, Morocco
  ...
  type stat_type
     ...
     real (dp) , dimension (:,:,:) , allocatable     :: vor_volume
     real (dp) , dimension (:,:,:) , allocatable     :: vor_shear
...
!! Benguerir, Morocco
  ...
  subroutine alloc_inst_type (inst)
    ...
    if ( ok > 0 ) stop 'error allocate inst type'

    if ( LES ) then

    allocate ( inst % mu_SGS( sx-ng:ex+ng , sy-ng:ey+ng , sz-ng:ez+ng )                   , & ! A.Techer
               inst % xiv2  ( sx-ng:ex+ng , sy-ng:ey+ng , sz-ng:ez+ng )                   , &
               ...
               inst % taumix  ( sx-ng:ex+ng , sy-ng:ey+ng , sz-ng:ez+ng )                 , &

               stat = ok )

    if ( ok > 0 ) stop 'error allocate inst type 2'
...
!
!    if ( ok /= 0 ) then
!       write (*,*) 'error opening ' , trim (dir_statvar) // trim (file_inst) // '_' // trim (number_ascii)
!       stop
!    end if
!
!
...
!! Benguerir, Morocco
  ...
  subroutine alloc_reyavg_type (reyavg)
    ...
    if ( ok > 0 ) stop 'error allocate reyavg type'
    ...
    if ( ok > 0 ) stop 'error allocate reyavg type 2'
...
!! Benguerir, Morocco
  ...
  subroutine write_reyavg_type (reyavg)
    ...
    if ( ok /= 0 ) then
       write (*,*) 'error opening ' , trim (dir_statvar) // trim (file_reyavg) // '_' // trim (number_ascii)
       stop
    end if
...
!! Benguerir, Morocco
  ...
  subroutine write_favavg_type (favavg)
    ...
    if ( ok /= 0 ) then
       write (*,*) 'error opening ' , trim (dir_statvar) // trim (file_favavg) // '_' // trim (number_ascii)
       stop
    end if
...
!! Benguerir, Morocco
  ...
  subroutine alloc_reyfluc_type (reyfluc)
    ...
    if ( ok > 0 ) stop 'error allocate reyfluc type'
...
!! Benguerir, Morocco
  ...
  subroutine alloc_favfluc_type (favfluc)
    ...
    if ( ok > 0 ) stop 'error allocate favfluc type'
...
!! Benguerir, Morocco
  ...
  subroutine alloc_stat_type (stat)
    ...
    allocate ( stat % reyavg_P      ( sx-ng:ex+ng , sy-ng:ey+ng , sz-ng:ez+ng )                     , &
               ...
               stat % vor_volume       ( sx-ng:ex+ng , sy-ng:ey+ng , sz-ng:ez+ng )                  , &
               stat % vor_shear        ( sx-ng:ex+ng , sy-ng:ey+ng , sz-ng:ez+ng )                  , &
    ...
    if ( ok > 0 ) stop 'error allocate stat type'
...
!! Benguerir, Morocco
  ...
  subroutine write_stat_type (stat)
    ...
    if ( ok /= 0 ) then
       write (*,*) 'error opening ' , trim (dir_statvar) // trim (file_stat) // '_' // trim (number_ascii)
       stop
    end if
    ...
    write (unit_stat) ((( stat % vor_volume (i,j,k) , i = sx , ex ) , &
                                                      j = sy , ey ) , &
                                                      k = sz , ez )
                                                   
    write (unit_stat) ((( stat % vor_shear (i,j,k) , i = sx , ex ) , &
                                                     j = sy , ey ) , &
                                                     k = sz , ez )
       ...
       do pz = 1 , inp % nprocz
          ...
          do py = 1 , inp % nprocy
             ...
             do px = 1 , inp % nprocx
                ...
                read (unit_stat) ((( stat % vor_volume (i,j,k) , i = ix , fx ) , &
                                                                 j = iy , fy ) , &
                                                                 k = iz , fz )
                                                               
                read (unit_stat) ((( stat % vor_shear (i,j,k) , i = ix , fx ) , &
                                                                j = iy , fy ) , &
                                                                k = iz , fz )
