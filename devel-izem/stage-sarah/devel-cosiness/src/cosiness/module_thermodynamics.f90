...
module thermodynamics
  ...
  integer (ip) , parameter , private        :: nptnegT_max   = 50  !< maximum number of points where T is negative

  real (dp) , parameter , private           :: eps_temp_mach = 1.0e-10_dp !< maximum temperature error (start simulation from restart file)
  real (dp) , parameter , private           :: eps_temp      = 1.0e-10_dp !< maximum temperature error (during simulation)
...
!> \brief Convert mass faction to molar fraction.
!!
!> \author
!! Modeling, Simulation and Data Analysis (MSDA)\n
!! Université Mohammed VI Polytechnique (UM6P)
  subroutine Y_to_X ( thd , Ya )


    type (thd_type) , intent (in)                                  :: thd !< thermodynamic derived type
    real (dp) , dimension (:) , intent (inout)                     :: Ya  !< mass (in) to molar (out) fraction
    ...
    wrk = 0.0_dp
    do l = 1 , nrv
       wrk = wrk + Ya (l) * thd % Wc_i (l)
    end do
    wrk = 1.0_dp / wrk


    do l = 1 , nrv
       Ya (l) = Ya (l) * thd % Wc_i (l) * wrk
    end do


  end subroutine Y_to_X
...
!! Université Mohammed VI Polytechnique (UM6P)
  subroutine prim_inv_var ( domain_id , thd , v , W_i , T , cp , ha )
    ...
    do while ( error > eps_temp .and. it < itmax )
          ...
          do j = j0 , j1
             do i = i0 , i1
                ...
                if ( T (i,j,k) < 0.0_dp ) then
                   ...
                   ipt = ipt + 1
                   if ( ipt > nptnegT_max ) call abort_mpi ('STOP: too many points with negative temperature')

                   ! Store points where T < 0
                   ir(ipt) = i ; jr(ipt) = j ; kr(ipt) = k

!                   npond = max ( 1 , min ( npond , i-i0 , i1-i ) ) !, j-j0 , j1-j , k-k0 , k1-k )
                   npond = 1
                   ndiv_i = 1.0_dp / float ( npond * ( npond + 1 ) )

!                   it = 0 ! Reset Newton iteration
                   correct_T = .true.
       ...
       if ( correct_T ) then
       ! Negative temperature correction consists in averaging the 
       ! conservatives variables and temperature on the left and right direction.

          ! Write a report in a file
          open (unit=13,file='report_negative_temp.out',status='unknown',position='append')

!          allocate ( avg ( ipt , nv+1 ) , stat = ok )
!          if ( ok > 0 ) call abort_mpi ('error allocate prim_inv_var 2')

          do l = 1 , ipt

              ! Averaging the conservtives variables
              do ivar = 1,nv
!                  v (ir(l),jr(l),kr(l),ivar) = 0.5_dp * ( v (ir(l)-1,jr(l),kr(l),ivar) + v (ir(l)+1,jr(l),kr(l),ivar) )

                  wrk = 0.0_dp
                  do ipond = 1 , npond
                     wrk = ipond * v (ir(l)-npond+ipond-1,jr(l),kr(l),ivar) &
                         + (npond-ipond+1) * v (ir(l)+ipond,jr(l),kr(l),ivar)
                  end do
!                  avg (l,ivar) = wrk * ndiv_i
                  v (ir(l),jr(l),kr(l),ivar) = wrk * ndiv_i
              end do
...
!             T (ir(l),jr(l),kr(l)) = 0.5_dp * ( T (ir(l)-1,jr(l),kr(l)) + T (ir(l)+1,jr(l),kr(l)) )
              wrk = 0.0_dp
              do ipond = 1 , npond
                 wrk = ipond * T (ir(l)-npond+ipond-1,jr(l),kr(l)) &
                     + (npond-ipond+1) * T (ir(l)+ipond,jr(l),kr(l))
              end do
!              avg (l,nv+1) = wrk * ndiv_i
              T (ir(l),jr(l),kr(l)) = wrk * ndiv_i

              ! Write in the report file
              write (13,'(1X,A,10(1X,I10))') '(it,l,ipt,domain_id,i,j,k)=' , &
                                              it , l , ipt , domain_id , ir(l) , jr(l) , kr(l)
              write (13,'(20(A10))') 'i','rho','rhou','rhov','rhow','rhoet',&
                                     'rhoY1','rhoY2','rhoY3','rhoY4','rhoY5','rhoY6','rhoY7','rhoY8','T'
              do i = i0 , i1
                 write (13,'(I5,20(1PE13.3))') i , v(i,jr(l),kr(l),1:nv) , T (i,jr(l),kr(l)) * thd % Tinf
              end do
...
!          do l = 1 , ipt
!              do ivar = 1,nv
!                  v (ir(l),jr(l),kr(l),ivar) = avg (l,ivar)
!              end do
!              T (ir(l),jr(l),kr(l)) = avg (l,nv+1)
...
!          deallocate (avg)
    ...
    if ( it >= itmax ) write (*,'(1X,A,1PE10.3)') 'WARNING: failure in temperatures convergence' , error

    if ( error /= error ) call abort_mpi ('STOP: error is a NaN')
...
!! Université Mohammed VI Polytechnique (UM6P)
  subroutine prim_inv_var_err_mach ( domain_id , thd , v , W_i , T , cp , ha )
    ...
    allocate ( DF ( i0:i1 , j0:j1 , k0:k1 ) , stat = ok )
    if ( ok > 0 ) call abort_mpi ('error allocate prim_inv_var_err_mach')
...
!! Université Mohammed VI Polytechnique (UM6P)
  subroutine newton ( domain_id , thd , v , T , cp , ha , F )
    ...
    allocate ( rho_i      ( i0:i1 , j0:j1 , k0:k1 )       , &
               ...
               stat = ok )
    if ( ok > 0 ) call abort_mpi ('error allocate newton')
...
!! Université Mohammed VI Polytechnique (UM6P)
  subroutine prim_vis_vars ( domain_id , thd , v , W_i , T , Xa , dm , mu , ct )
    ...
    allocate ( DiffKJ        ( i0:i1 , j0:j1 , k0:k1 , 1:ns , 1:ns ) , &
               ...
               stat = ok )
    if ( ok > 0 ) call abort_mpi ('error allocate prim_vis_vars')
...
!> \brief Calculate primitive viscid variables (except viscosity).
!!
!! It provides optimization to the code.
!!
!> \author
...
!! Université Mohammed VI Polytechnique (UM6P)
  subroutine prim_vis_vars_wo_mu ( domain_id , thd , v , W_i , T , Xa , dm , ct )
    ...
    allocate ( DiffKJ        ( i0:i1 , j0:j1 , k0:k1 , 1:ns , 1:ns ) , &
               ...
               wrk2          ( i0:i1 , j0:j1 , k0:k1 , 1:ns        ) , &
               stat = ok )
    if ( ok > 0 ) call abort_mpi ('error allocate prim_vis_vars_wo_mu')
...
!! Université Mohammed VI Polytechnique (UM6P)
  subroutine prim_vis_ct ( domain_id , thd , T , Xa , ct )
    ...
    allocate ( logtempkelvin ( 1:npoly , i0:i1 , j0:j1 , k0:k1     ) , &
               ...
               wrk2          ( i0:i1 , j0:j1 , k0:k1 , 1:ns        ) , &
               stat = ok )
    if ( ok > 0 ) call abort_mpi ('error allocate prim_vis_ct')
...
!! Université Mohammed VI Polytechnique (UM6P)
  subroutine prim_vis_Dm ( domain_id , thd , v , W_i , T , Xa , dm )
    ...
    allocate ( DiffKJ        ( i0:i1 , j0:j1 , k0:k1 , 1:ns , 1:ns ) , &
               wrk1          ( i0:i1 , j0:j1 , k0:k1               ) , &
               stat = ok )
    if ( ok > 0 ) call abort_mpi ('error allocate prim_vis_Dm')
