...
!! Modeling, Simulation and Data Analysis (MSDA)\n
!! Université Mohammed VI Polytechnique (UM6P)
  subroutine averages ( ifile , inp , thd , adi , sim , grid , dt , inst , reyavg , favavg )
    ...
    dy (sy:ey)                = 1.0_dp / grid % dy_i (sy:ey)
    dz (sz:ez)                = 1.0_dp / grid % dz_i (sz:ez)
    ...
    do spc = 1 , nss
       ...
       wrk3 (sx:ex,sy:ey,sz:ez) = reyavg % Yass (sx:ex,sy:ey,sz:ez,spc)
       wrk4 (sx:ex,sy:ey,sz:ez) = favavg % Yass (sx:ex,sy:ey,sz:ez,spc)
       call spatZ_avg ( inp % spat_avg , dz , wrk1 )
       call spatZ_avg ( inp % spat_avg , dz , wrk2 )
       call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , wrk3 )
       call temp_avg ( fav_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk2 , wrk4 )
       reyavg % Yass (sx:ex,sy:ey,sz:ez,spc) = wrk3 (sx:ex,sy:ey,sz:ez)
       favavg % Yass (sx:ex,sy:ey,sz:ez,spc) = wrk4 (sx:ex,sy:ey,sz:ez)
    end do
...
!       call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , wrk2 , wrk1 , reyavg % taumix )
    ...
    if ( LES ) then 
       ...
       ! tke_SGS
       call tke_SGS_yoshizawa ( adi , inst % mu_SGS , rho , grid % delta , wrk1 )
       wrk2 (sx:ex,sy:ey,sz:ez) = wrk1 (sx:ex,sy:ey,sz:ez) * rho (sx:ex,sy:ey,sz:ez)
       call spatZ_avg ( inp % spat_avg , dz , wrk1 )
       call spatZ_avg ( inp % spat_avg , dz , wrk2 )
!       call temp_avg ( fav_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , reyavg % tke_SGS ) ! need to create reyavg % tke_SGS, if you want reyavg
       call temp_avg ( fav_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk2 , favavg % tke_SGS )

       ! tau_iso_SGS : Isotropic part of the SGS viscous stress tensor
       call tau_iso_SGS_selector_post ( inp , grid , inst % u , wrk1 )
       call spatZ_avg ( inp % spat_avg , dz , wrk1 )
       call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , reyavg % tau_iso_SGS )
       ...
       call tau_ij_SGS ( inp , adi , grid , inst % u , wrk1 , wrk2 , wrk3 , inst % mu_SGS , wrk_dim5 )
       ! tau_11
       wrk1 (sx:ex,sy:ey,sz:ez) = wrk_dim5 (sx:ex,sy:ey,sz:ez,1,1)
       wrk2 (sx:ex,sy:ey,sz:ez) = reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,1,1)
       call spatZ_avg ( inp % spat_avg , dz , wrk1 )
       call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , wrk2 )
       reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,1,1) = wrk2 (sx:ex,sy:ey,sz:ez)
       ! tau_12
       wrk1 (sx:ex,sy:ey,sz:ez) = wrk_dim5 (sx:ex,sy:ey,sz:ez,1,2)
       wrk2 (sx:ex,sy:ey,sz:ez) = reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,1,2)
       call spatZ_avg ( inp % spat_avg , dz , wrk1 )
       call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , wrk2 )
       reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,1,2) = wrk2 (sx:ex,sy:ey,sz:ez)
       ! tau_13
       wrk1 (sx:ex,sy:ey,sz:ez) = wrk_dim5 (sx:ex,sy:ey,sz:ez,1,3)
       wrk2 (sx:ex,sy:ey,sz:ez) = reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,1,3)
       call spatZ_avg ( inp % spat_avg , dz , wrk1 )
       call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , wrk2 )
       reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,1,3) = wrk2 (sx:ex,sy:ey,sz:ez)
       ! tau_21
       reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,2,1) = reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,1,2)
       ! tau_22
       wrk1 (sx:ex,sy:ey,sz:ez) = wrk_dim5 (sx:ex,sy:ey,sz:ez,2,2)
       wrk2 (sx:ex,sy:ey,sz:ez) = reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,2,2)
       call spatZ_avg ( inp % spat_avg , dz , wrk1 )
       call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , wrk2 )
       reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,2,2) = wrk2 (sx:ex,sy:ey,sz:ez)
       ! tau_23
       wrk1 (sx:ex,sy:ey,sz:ez) = wrk_dim5 (sx:ex,sy:ey,sz:ez,2,3)
       wrk2 (sx:ex,sy:ey,sz:ez) = reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,2,3)
       call spatZ_avg ( inp % spat_avg , dz , wrk1 )
       call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , wrk2 )
       reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,2,3) = wrk2 (sx:ex,sy:ey,sz:ez)
       ! tau_31
       reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,3,1) = reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,1,3)
       ! tau_32
       reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,3,2) = reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,2,3)
       ! tau_33
       wrk1 (sx:ex,sy:ey,sz:ez) = wrk_dim5 (sx:ex,sy:ey,sz:ez,3,3)
       wrk2 (sx:ex,sy:ey,sz:ez) = reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,3,3)
       call spatZ_avg ( inp % spat_avg , dz , wrk1 )
       call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , wrk2 )
       reyavg % tau_SGS (sx:ex,sy:ey,sz:ez,3,3) = wrk2 (sx:ex,sy:ey,sz:ez)
    ...
    ! Ydot
    do spc = 1 , nrv
       wrk1 (sx:ex,sy:ey,sz:ez) = inst % Ydot_hct (sx:ex,sy:ey,sz:ez,spc)
       wrk2 (sx:ex,sy:ey,sz:ez) = wrk1 (sx:ex,sy:ey,sz:ez) * rho (sx:ex,sy:ey,sz:ez)
       wrk3 (sx:ex,sy:ey,sz:ez) = reyavg % Ydot_hct (sx:ex,sy:ey,sz:ez,spc)
       wrk4 (sx:ex,sy:ey,sz:ez) = favavg % Ydot_hct (sx:ex,sy:ey,sz:ez,spc)
       call spatZ_avg ( inp % spat_avg , dz , wrk1 )
       call spatZ_avg ( inp % spat_avg , dz , wrk2 )
       call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , wrk3 )
       call temp_avg ( fav_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk2 , wrk4 )
       reyavg % Ydot_hct (sx:ex,sy:ey,sz:ez,spc) = wrk3 (sx:ex,sy:ey,sz:ez)
       favavg % Ydot_hct (sx:ex,sy:ey,sz:ez,spc) = wrk4 (sx:ex,sy:ey,sz:ez)
    end do
    ...
    ! omega_hct
    wrk1 (sx:ex,sy:ey,sz:ez) = inst % omega_hct (sx:ex,sy:ey,sz:ez)
    wrk2 (sx:ex,sy:ey,sz:ez) = wrk1 (sx:ex,sy:ey,sz:ez) * rho (sx:ex,sy:ey,sz:ez)
    call spatZ_avg ( inp % spat_avg , dz , wrk1 )
    call spatZ_avg ( inp % spat_avg , dz , wrk2 )
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , reyavg % omega_hct )
    call temp_avg ( fav_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk2 , favavg % omega_hct )
    ...
    ! Damkohler II (with the reactivity rate)
    ...
    call spatZ_avg ( inp % spat_avg , dz , wrk1 )
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , reyavg % Da_II )
    ...
    call D_equivalent ( inp , thd , grid % dx_i , grid % dy_i , grid % dz_i , inst % u , inst % T , inst % W_i , &
    ...
    ! Premixed combustion zone probability
    call proba_PZ ( wrk1 , wrk2 ) ! wrk1 = FI calculated previously
    wrk1 (sx:ex,sy:ey,sz:ez) = wrk2 (sx:ex,sy:ey,sz:ez)
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , reyavg % proba_pz )
    ...
    call spatZ_avg ( inp % spat_avg , dz , wrk1 )
    call spatZ_avg ( inp % spat_avg , dz , wrk2 )
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , reyavg % lambda )
    call temp_avg ( fav_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk2 , favavg % lambda )
    ...
    wrk2 (sx:ex,sy:ey,sz:ez) = inst % u (sx:ex,sy:ey,sz:ez,3) * rho_i (sx:ex,sy:ey,sz:ez)
    wrk3 (sx:ex,sy:ey,sz:ez) = inst % u (sx:ex,sy:ey,sz:ez,4) * rho_i (sx:ex,sy:ey,sz:ez)
    call tau_ij ( grid % dx_i , grid % dy_i , grid % dz_i , wrk1 , wrk2 , wrk3 , inst % mu , inst % kpa , wrk_dim5 )
    ...
    wrk2 (sx:ex,sy:ey,sz:ez) = inst % Da (sx:ex,sy:ey,sz:ez) * rho (sx:ex,sy:ey,sz:ez)
    call cond_avg ( fav_avg , ifile , end_file , dtime , sumdtime , dy , dz , &
                    min_Y , max_Y ,                                           &
    ...
    call takeno ( inp , grid % dx_i , grid % dy_i , grid % dz_i , inst % omega , inst % u , wrk2 )
    ...
    wrk1 (sx:ex,sy:ey,sz:ez) = log10 ( inst % Da (sx:ex,sy:ey,sz:ez) )
    call cond_avg ( rey_avg , ifile , end_file , dtime , sumdtime , dy , dz , &
...
!! Modeling, Simulation and Data Analysis (MSDA)\n
!! Université Mohammed VI Polytechnique (UM6P)
  subroutine spectras ( ifile , inp , thd , sim , grid , inst , stat )
...
!! Université Mohammed VI Polytechnique (UM6P)
  subroutine fluctuations ( inp , thd , adi , dx_i , dy_i , dz_i , inst , reyavg , favavg , reyfluc , favfluc )
    ...
    do spc = 1 , nss
       wrk1 (sx:ex,sy:ey,sz:ez) = inst % Yass (sx:ex,sy:ey,sz:ez,spc)
       reyfluc % Yass (sx:ex,sy:ey,sz:ez,spc) = wrk1 (sx:ex,sy:ey,sz:ez) - reyavg % Yass (sx:ex,sy:ey,sz:ez,spc)
       favfluc % Yass (sx:ex,sy:ey,sz:ez,spc) = wrk1 (sx:ex,sy:ey,sz:ez) - favavg % Yass (sx:ex,sy:ey,sz:ez,spc)
    end do
    ...
    ! Ydot
    do spc = 1 , nrv
       reyfluc % Ydot (sx:ex,sy:ey,sz:ez,spc) = &
       & inst % Ydot (sx:ex,sy:ey,sz:ez,spc) - reyavg % Ydot (sx:ex,sy:ey,sz:ez,spc)
       favfluc % Ydot (sx:ex,sy:ey,sz:ez,spc) = &
       & inst % Ydot (sx:ex,sy:ey,sz:ez,spc) - favavg % Ydot (sx:ex,sy:ey,sz:ez,spc)
    end do

    ! Ydot_hct
    do spc = 1 , nrv
       reyfluc % Ydot_hct (sx:ex,sy:ey,sz:ez,spc) = &
       & inst % Ydot_hct (sx:ex,sy:ey,sz:ez,spc) - reyavg % Ydot_hct (sx:ex,sy:ey,sz:ez,spc)
       favfluc % Ydot_hct (sx:ex,sy:ey,sz:ez,spc) = &
       & inst % Ydot_hct (sx:ex,sy:ey,sz:ez,spc) - favavg % Ydot_hct (sx:ex,sy:ey,sz:ez,spc)
    end do


    ! omega
    reyfluc % omega (sx:ex,sy:ey,sz:ez) = &
    & inst % omega (sx:ex,sy:ey,sz:ez) - reyavg % omega (sx:ex,sy:ey,sz:ez)
    favfluc % omega (sx:ex,sy:ey,sz:ez) = &
    & inst % omega (sx:ex,sy:ey,sz:ez) - favavg % omega (sx:ex,sy:ey,sz:ez)

    reyfluc % omega_hct (sx:ex,sy:ey,sz:ez) = &
    & inst % omega_hct (sx:ex,sy:ey,sz:ez) - reyavg % omega_hct (sx:ex,sy:ey,sz:ez)
    favfluc % omega_hct (sx:ex,sy:ey,sz:ez) = &
    & inst % omega_hct (sx:ex,sy:ey,sz:ez) - favavg % omega_hct (sx:ex,sy:ey,sz:ez)


    ! Da
    reyfluc % Da (sx:ex,sy:ey,sz:ez) = &
    & inst % Da (sx:ex,sy:ey,sz:ez) - reyavg % Da (sx:ex,sy:ey,sz:ez)
    favfluc % Da (sx:ex,sy:ey,sz:ez) = &
    & inst % Da (sx:ex,sy:ey,sz:ez) - favavg % Da (sx:ex,sy:ey,sz:ez)
    ...
    ! Da_Zm
    call cond_fluc ( min_Y , max_Y , wrk1 , inst % Da , favavg % ca_Da_Zm , favfluc % ca_Da_Zm )
...
!! Modeling, Simulation and Data Analysis (MSDA)\n
!! Université Mohammed VI Polytechnique (UM6P)
  subroutine stats ( ifile , inp , thd , adi , sim , grid , dt , &
    ...
    real (dp) , dimension (:) , allocatable , intent (in)          :: dt      !< time step
    type (inst_type) , intent (in)                                 :: inst    !< instantaneous derived type
    type (reyavg_type) , intent (inout)                            :: reyavg  !< Reynolds average derived type       ! intent (in) ONLY ?
    type (favavg_type) , intent (inout)                            :: favavg  !< Favre average derived type          ! intent (in) ONLY ?
    type (reyfluc_type) , intent (inout)                           :: reyfluc !< Reynolds fluctuation derived type   ! intent (in) ONLY ?
    type (favfluc_type) , intent (inout)                           :: favfluc !< Favre fluctuation derived type      ! intent (in) ONLY ?
    type (stat_type) , intent (inout)                              :: stat    !< statistical derived type
    ...
    ! YssYss
    do spc = 1 , nss
       wrk1 (sx:ex,sy:ey,sz:ez) = reyfluc % Yass (sx:ex,sy:ey,sz:ez,spc) * reyfluc % Yass (sx:ex,sy:ey,sz:ez,spc)
       wrk2 (sx:ex,sy:ey,sz:ez) = favfluc % Yass (sx:ex,sy:ey,sz:ez,spc) * favfluc % Yass (sx:ex,sy:ey,sz:ez,spc) * &
                                  rho (sx:ex,sy:ey,sz:ez)
       wrk3 (sx:ex,sy:ey,sz:ez) = stat % reyvar_Yass (sx:ex,sy:ey,sz:ez,spc)
       wrk4 (sx:ex,sy:ey,sz:ez) = stat % favvar_Yass (sx:ex,sy:ey,sz:ez,spc)
       call spatZ_avg ( inp % spat_avg , dz , wrk1 )
       call spatZ_avg ( inp % spat_avg , dz , wrk2 )
       call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , wrk3 )
       call temp_avg ( fav_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk2 , wrk4 )
       stat % reyvar_Yass (sx:ex,sy:ey,sz:ez,spc) = wrk3 (sx:ex,sy:ey,sz:ez)
       stat % favvar_Yass (sx:ex,sy:ey,sz:ez,spc) = wrk4 (sx:ex,sy:ey,sz:ez)
    end do
    ...
    call comm_one (wrk1) ; call dx ( grid % dx_i , wrk1 , wrk2 )
    ...
    ! dissipation (always assess it, because you need it to evaluate some other variables)
    call tke_dissipation ( grid % dx_i , grid % dy_i , grid % dz_i , favfluc % ux , favfluc % vy , favfluc % wz , &
                           reyfluc % tau , wrk1 )
    call spatZ_avg ( inp % spat_avg , dz , wrk1 )
    ...
    ! vorticity vector
    call vorticity_i ( grid % dx_i , grid % dy_i , grid % dz_i , inst % u , wrk_dim4 )

    ! convection
    call vorticity_convection ( grid % dx_i , grid % dy_i , grid % dz_i , inst % u , wrk_dim4 , wrk1 )
    call spatZ_avg ( inp % spat_avg , dz , wrk1 )
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , stat % vor_conv )

    call enstrophy_convection ( grid % dx_i , grid % dy_i , grid % dz_i , inst % u , wrk_dim4 , wrk1 )
    call spatZ_avg ( inp % spat_avg , dz , wrk1 )
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , stat % ens_conv )

    ! stretching
    call vorticity_stretching ( grid % dx_i , grid % dy_i , grid % dz_i , inst % u , wrk_dim4 , wrk1 )
    call spatZ_avg ( inp % spat_avg , dz , wrk1 )
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , stat % vor_stret )

    call enstrophy_stretching ( grid % dx_i , grid % dy_i , grid % dz_i , inst % u , wrk_dim4 , wrk1 )
    call spatZ_avg ( inp % spat_avg , dz , wrk1 )
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , stat % ens_stret )

    ! dilatation
    call vorticity_dilatation ( grid % dx_i , grid % dy_i , grid % dz_i , inst % u , wrk_dim4 , wrk1 )
    call spatZ_avg ( inp % spat_avg , dz , wrk1 )
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , stat % vor_dila )

    call enstrophy_dilatation ( grid % dx_i , grid % dy_i , grid % dz_i , inst % u , wrk_dim4 , wrk1 )
    call spatZ_avg ( inp % spat_avg , dz , wrk1 )
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , stat % ens_dila )

    ! baroclinic
    call vorticity_baroclinic ( grid % dx_i , grid % dy_i , inst % W_i , inst % T , inst % u , wrk1 )
    call spatZ_avg ( inp % spat_avg , dz , wrk1 )
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , stat % vor_baro )

    call enstrophy_baroclinic ( grid % dx_i , grid % dy_i , dz , inst % W_i , inst % T , inst % u , wrk_dim4 , wrk1 )
    call spatZ_avg ( inp % spat_avg , dz , wrk1 )
    ...
    call vorticity_viscous ( grid % dx_i , grid % dy_i , grid % dz_i , inst % u , wrk_dim5 , wrk1 )
    call spatZ_avg ( inp % spat_avg , dz , wrk1 )
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , stat % vor_visc )

    call enstrophy_viscous ( grid % dx_i , grid % dy_i , grid % dz_i , inst % u , wrk_dim4 , wrk_dim5 , wrk1 )
    call spatZ_avg ( inp % spat_avg , dz , wrk1 )
    ...
    ! molecular diffusion
    call scal_var_mol_diff ( inp % index_scalar , thd , grid % dx_i , grid % dy_i , grid % dz_i , inst % T , inst % W_i , &
    ...
    call scal_var_dissip ( inp % index_scalar , thd , grid % dx_i , grid % dy_i , grid % dz_i , inst % T , inst % W_i , &
    ...
    ! ux
    call correlation_Z ( inp , grid , reyfluc % ux , wrk1 )
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , stat % tmp_corr_ux )
    if ( ifile == end_file ) call last_correlation_Z ( inp , grid , stat % tmp_corr_ux )

    ! vy
    call correlation_Z ( inp , grid , reyfluc % vy , wrk1 )
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , stat % tmp_corr_vy )
    if ( ifile == end_file ) call last_correlation_Z ( inp , grid , stat % tmp_corr_vy )

    ! wz
    call correlation_Z ( inp , grid , reyfluc % wz , wrk1 )
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , stat % tmp_corr_wz )
    if ( ifile == end_file ) call last_correlation_Z ( inp , grid , stat % tmp_corr_wz )

    ! p
    call correlation_Z ( inp , grid , reyfluc % p , wrk1 )
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk1 , stat % tmp_corr_p )
    if ( ifile == end_file ) call last_correlation_Z ( inp , grid , stat % tmp_corr_p )
    ...
    call correlation_Z ( inp , grid , wrk1 , wrk2 )
    call temp_avg ( rey_avg , ifile , end_file , dtime , sumdtime , reyavg % rho , wrk2 , stat % tmp_corr_Y )
    if ( ifile == end_file ) call last_correlation_Z ( inp , grid , stat % tmp_corr_Y )
    ...
    if ( ifile == end_file ) call last_correlation_Z ( inp , grid , stat % tmp_corr_Zm )
