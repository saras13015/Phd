...
C----------------------------------------------------------------------C
C  3-Step (or 4-Step) Reduced Mechanism for H2-O2 Systems
C  5(+1) species: H2  O2  H2O  H  X  (N2 diluent)
C  2(+1) SPECIES: O  OH  (H2O2 not used)
C         3H2 + O2   = 2H2O + 2H    (R1)
C         H + H + M  = H2 + M       (R2)
C         H2 + O2    = X + H        (R3)
C
C  If T < Tc, then H and HO2 are in steady state (SS), so
C         3H2 + O2   = 2H2O + 2H    (R1-)
C         H + H + M  = H2 + M       (R2-)
C         H2 + O2    = H2O2         (R3-)
C  else, H2O2 is in SS, so
C         3H2 + O2   = 2H2O + 2H    (R1+)
C         H + H + M  = H2 + M       (R2+)
C         H2 + O2    = HO2 + H      (R3+)
C
...
C
      ...
      DIMENSION RKF(II), RKR(II), RROP(II)

!      INCLUDE 'myvar.h'                                                   ! include: if you want to extract SS_k and SLAMBDA
...
C-- 5+1 species:  H2     O2      H2O     H    X=HO2     (N2)
...
C      DO K = 1, KK
C        XCON(K) = RHO*Y(K) / WT(K) ! = rho*Y_k/W_k = molar concentration (mol/cm^-3)
C      END DO
...
C       STEADY STATE OF OH (PCI_2011,eq.3)
...
C       Meth 2: SLAMBDA from CF_2012,eq.5
        ...
        IF ( AA < 0.0D0 ) write(*,*) 
     &   'ERROR: in CKWYP/CKWYR, SLAMBDA AA is negative' , AA
