...
!! Benguerir, Morocco
  ...
  subroutine vtr_paraview ( ifile , inp , thd , adi , sim , x , y , z , xt , yt , zt , &
                            dx_i , dy_i , dz_i , inst , reyavg , favavg , stat )
...
! call exit(0)
    ...
    if ( inp % nondim_grid ) then
       write (unit_plot) nx*nbitsimple , real ( x (ix:fx) * adi % L_ref / inp % length_ref )
       write (unit_plot) ny*nbitsimple , real ( y (iy:fy) * adi % L_ref / inp % length_ref )
       write (unit_plot) nz*nbitsimple , real ( z (iz:fz) * adi % L_ref / inp % length_ref )
    else
       write (unit_plot) nx*nbitsimple , real ( x (ix:fx) * adi % L_ref , sp )
       write (unit_plot) ny*nbitsimple , real ( y (iy:fy) * adi % L_ref , sp )
       write (unit_plot) nz*nbitsimple , real ( z (iz:fz) * adi % L_ref , sp )
    end if

    ! plotting variables
    if ( .not. inp % temp_avg .and. .not. inp % spat_avg ) then ! instantaneous plot
       do iv = 1 , inp % nvars
          call plot_inst_var ( ifile , iv , inp , thd , adi , sim , dx_i , dy_i , dz_i , inst )
       end do
    else ! statistics plot
       do iv = 1 , inp % nvars
          call plot_stat_var ( iv , inp , thd , adi , x , y , z , xt , yt , zt , &
...
!! Benguerir, Morocco
  ...
  subroutine pvtr_paraview ( ifile , inp )
    ...
    if ( rank == rank_default ) then
       ...
       write (*,*) 'file ' , trim (file_plot) // trim (file_ascii) , ' written'
  ...
  subroutine pvd_paraview ( inp , adi , sim , t )
    ...
    if ( rank == rank_default ) then
       ...
       if  ( inp % read_stat ) then
          start_file = sim % correct_endfile
          end_file   = sim % correct_endfile
       else
