...
C
      PARAMETER (MDIM=10, KDIM=500, MKDIM=MDIM*KDIM, IDIM=5000, LSYM=16,
...
C
  ...
  500    CONTINUE
C
         WRITE (LOUT, '(/1X,A)') ' NOTE: '//IUNITS(:ILASCH2(IUNITS))
C
      ELSEIF (THERMO) THEN
C
...
C
      DO 200 N = 1, NSUB
         ...
         IF (I1 .EQ .1) THEN
            KERR = .TRUE.
            WRITE (LOUT, 130) SUB(N)(:ILASCH2(SUB(N)))
         ELSE
            IF (I1 .LE. 0) THEN
               ISTR = SUB(N)
            ELSE
...
C
            IF (INUM .GT. 0) THEN
               WRITE (LOUT, 100) SUB(N)(:ILASCH2(SUB(N)))
            ELSE
               IF (NN .LT. NDIM) THEN
                  IF (ISTR(ILEN+1:) .NE. ' ') THEN
                     WRITE (LOUT, 120) SUB(N)(:ILASCH2(SUB(N)))
                     KERR = .TRUE.
                  ELSE
                     NN = NN + 1
                     STRAY(NN) = ' '
                  ...
                  ENDIF
               ELSE
                  WRITE (LOUT, 110) SUB(N)(:ILASCH2(SUB(N)))
                  KERR = .TRUE.
               ENDIF
            ENDIF
         ENDIF
...
C
   ...
   25 CONTINUE
C
      IF (ILASCH2(INAME).GE.45 .AND. I1.GT.0) THEN
...
C
            ...
            IF (KNUM .EQ. 0) THEN
               ...
               WRITE (LOUT, 680) ISPEC(:ILASCH2(ISPEC))
               KERR = .TRUE.
            ELSE
C
...
C
            ...
            IF (K .EQ. 0) THEN
               WRITE (LOUT, 1040) KEY(:ILASCH2(KEY))
               KERR = .TRUE.
            ELSE
               IF (.NOT.LTHB) THEN
                  KERR = .TRUE.
...
C
C   REFERENCES  (NONE)
C   ROUTINES CALLED  IFIRCH,ILASCH2
...
C  When used with the companion routine ILASCH2, the length of a string
...
120   CONTINUE
      IFIRCH = I
      END
      FUNCTION ILASCH2(STRING)
C   BEGIN PROLOGUE  ILASCH2
C   DATE WRITTEN   850626
C   REVISION DATE  850626
C   CATEGORY NO.  M4.
C   KEYWORDS  CHARACTER STRINGS,SIGNIFICANT CHARACTERS
...
C  arbitrary length.  If no characters are found, ILASCH2 is set = 0.
...
C
      CHARACTER*(*) STRING
C
C***FIRST EXECUTABLE STATEMENT ILASCH2
      NLOOP = LEN(STRING)
      IF (NLOOP.EQ.0) THEN
         ILASCH2 = 0
         RETURN
