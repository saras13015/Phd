...
!! Benguerir, Morocco
  ...
  subroutine init_eglib ( inp , rweg , iweg )
    ...
    allocate ( iweg (liweg) , rweg (lrweg) , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate init_eglib'
...
!! Benguerir, Morocco
  ...
  subroutine egvars ( thd , T , W_i , v , iweg , rweg , rd , mu , kpa , ct , tdr )
    ...
    real (dp)                                     :: breakflagmin , breakflagmax
    real (dp) , dimension (npoly+1)               :: tempkelvin
    ! EGLIB variables
    real (dp)                                     :: T_eg , ww_eg , ct_eg , ct_eg_dummy, mu_eg , kpa_eg
    real (dp) , dimension (nrv)                   :: Y_eg , X_eg , cp_eg , tdr_eg,  tdr_eg_dummy
...
!                   call egsdr1  ( T_eg , Y_eg , ww_eg , rweg , rd_eg )
                   ...
                   ! radouan
!                   call egsl4   ( T_eg , Y_eg , rweg , iweg , ct_eg )     
                   call egse3   ( T_eg , Y_eg , rweg , mu_eg )                         ! shear viscosity
!                   call egsk3   ( T_eg , Y_eg , rweg , kpa_eg )                        ! volume viscosity
                   call egslct1 ( T_eg , Y_eg , rweg , iweg , ct_eg , tdr_eg_dummy )   ! thermal conductivity
                   call egslct3 ( T_eg , Y_eg , rweg , iweg , ct_eg_dummy , tdr_eg )   ! rescaled thermal diffusion
                   call egsdr1  ( T_eg , Y_eg , ww_eg , rweg , rd_eg )                 ! rescaled flux diffusion matrix
...
!! Benguerir, Morocco
  ...
  subroutine viscflux_eg_x ( thd , adi , dx_i , T , W_i , cp , ha , v , &
                             rweg , iweg , fl )
    ...
    allocate ( rd  (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng,nrv,nrv) , &
               ...
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_x'
    ...
    allocate ( ux (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)         , &
               ...
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_x 2'
    ...
    allocate ( sx_x  (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng) , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_x 3'
    ...
    allocate ( q_x (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng) , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_x 4'
    ...
    allocate ( wrka_x (sx:ex,sy:ey,sz:ez,nrv+npv) , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_x 5'
...
!! Benguerir, Morocco
  ...
  subroutine viscflux_eg_LES_x ( inp , thd , adi , dx_i , T , W_i , cp , ha , v , &
                                 rweg , iweg , fl )
    ...
    integer (ip)                                    :: ok , domain_id , i , j , k , l , m
    integer (ip)                                    :: i0 , i1 , j0 , j1 , k0 , k1
    real (dp)                                       :: rmu , rmu_SGS , rtau_iso_SGS , rmk , rvd0 , rlam0 , rlam , rlamcp0 , bulk , &
                                                       Sc_SGS_i , Pr_SGS_i , gm1_g , ct_SGS , dm_SGS , rq1 , rq2
    real (dp)                                       :: rho_i , Ps , Ts , uxs
    real (dp) , dimension (nrv)                     :: d_x
    real (dp)                                       :: wrk1 , wrk2
    ! viscous variables (eglib)
    ...
    allocate ( rd  (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng,nrv,nrv) , &
               ...
               ct  (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)         , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_LES_x'
    ...
    allocate ( ux (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)         , &
               ...
               fd (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng,nderiv)  , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_LES_x 2'
    ...
    allocate ( sx_x         (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)      , &
               ...
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_LES_x 3'
       ...
       do k = k0 , k1
          do j = j0 , j1
             do i = i0 , i1
                ...
                rtau_iso_SGS = adi % sqgmr * tau_iso_SGS (i,j,k)
                rmk          = - adi % sqgmr * kpa (i,j,k)

                bulk = rmk * fd (i,j,k,1)

                sx_x (i,j,k) = bulk + twothirds * rmu * ( fd (i,j,k,1) + fd (i,j,k,1) )
                sx_x_SGS (i,j,k) = bulk + ( sx_x (i,j,k) - bulk ) * ( 1.0_dp + rmu_SGS ) + rtau_iso_SGS
       ...
       do k = k0 , k1
          do j = j0 , j1
             do i = i0 , i1
                ...
                dm_SGS = mu_SGS (i,j,k) * rho_i * Sc_SGS_i
                ...
                do l = 1 , nrv
                   ...
                   ! plus two additional SGS terms
                   wrk1 = v(i,j,k,1) * dm_SGS * adi % Sc * thd % Wc (l)

                   fd (i,j,k,nfluid+ndim*l) = - rvd0 * ( fd (i,j,k,nfluid+ndim*l  ) * ( 1.0_dp + wrk1 * W_i (i,j,k) ) + &
                                                           wrk1 * Xa (i,j,k,l) * fd (i,j,k,nSGS+1) )
    ...
    allocate ( q_x (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng) , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_LES_x 4'
       ...
       do k = k0 , k1
          do j = j0 , j1
             do i = i0 , i1
                ...
                ct_SGS = mu_SGS (i,j,k) * cp (i,j,k) * Pr_SGS_i

                rlam = -rlam0 * ( ct (i,j,k) + ct_SGS )
                rq1  = rq1 * rlam0 * adi % sqgmr
                rq2  = rq2 * rlam0 * gm1_g
    ...
    allocate ( wrka_x (sx:ex,sy:ey,sz:ez,nrv+npv) , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_LES_x 5'
...
!! Benguerir, Morocco
  ...
  subroutine viscflux_eg_xy ( thd , adi , dx_i , dy_i , T , W_i , cp , ha , v , &
                              rweg , iweg , fl )
    ...
    allocate ( rd  (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng,nrv,nrv) , &
               ...
               ct  (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)         , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_xy'
    ...
    allocate ( ux (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)         , &
               ...
               fd (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng,nderiv)  , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_xy 2'
    ...
    allocate ( sx_x  (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng) , &
               ...
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_xy 3'
    ...
    allocate ( q_x (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng) , &
               ...
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_xy 4'
    ...
    allocate ( wrka_x (sx:ex,sy:ey,sz:ez,nrv+npv) , &
               ...
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_xy 5'
...
!! Benguerir, Morocco
  ...
  subroutine viscflux_eg_LES_xy ( inp , thd , adi , dx_i , dy_i , T , W_i , cp , ha , v , &
                                  rweg , iweg , fl )
    ...
    allocate ( rd  (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng,nrv,nrv) , &
               ...
               ct  (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)         , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_LES_xy'
    ...
    allocate ( ux (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)         , &
               ...
               fd (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng,nderiv)  , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_LES_xy 2'
    ...
    call dy_fixed1 ( dy_i , W_i  (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)      , &
                            fd (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng,nSGS+2) )
    ...
    allocate ( sx_x         (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)      , &
               ...
               dtau_iso_SGS (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng,ndim) , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_LES_xy 3'
       ...
       do k = k0 , k1
          do j = j0 , j1
             do i = i0 , i1
                ...
                sy_y (i,j,k) = bulk + twothirds * rmu * ( fd (i,j,k,4) + fd (i,j,k,4) - fd (i,j,k,1) )
                sy_y_SGS (i,j,k) = bulk + ( sy_y (i,j,k) - bulk ) * ( 1.0_dp + rmu_SGS ) + rtau_iso_SGS
          ...
          do j = j0 , j1
             do i = i0 , i1
                ...
                do l = 1 , nrv
                   ...
                   fd (i,j,k,nfluid+ndim*l-1) = - rvd0 * ( fd (i,j,k,nfluid+ndim*l-1) * ( 1.0_dp + wrk1 * W_i (i,j,k) ) + &
                                                           wrk1 * Xa (i,j,k,l) * fd (i,j,k,nSGS+1) )

                   fd (i,j,k,nfluid+ndim*l  ) = - rvd0 * ( fd (i,j,k,nfluid+ndim*l  ) * ( 1.0_dp + wrk1 * W_i (i,j,k) ) + &
                                                           wrk1 * Xa (i,j,k,l) * fd (i,j,k,nSGS+2) )
    ...
    allocate ( q_x (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng) , &
               q_y (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng) , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_LES_xy 4'
    ...
    deallocate ( ct , tdr ,                            &
                 mu_SGS , tau_iso_SGS , dtau_iso_SGS , &
                                         sx_x , sx_y , &
                                                sy_y )
    ...
    allocate ( wrka_x (sx:ex,sy:ey,sz:ez,nrv+npv) , &
               wrka_y (sx:ex,sy:ey,sz:ez,nrv+npv) , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_LES_xy 5'
...
!! Benguerir, Morocco
  ...
  subroutine viscflux_eg_xyz ( thd , adi , dx_i , dy_i , dz_i , T , W_i , cp , ha , v , &
                               rweg , iweg , fl )
    ...
    allocate ( rd  (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng,nrv,nrv) , &
               ...
               ct  (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)         , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_xyz'
    ...
    allocate ( ux (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)         , &
               ...
               fd (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng,nderiv)  , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_xyz 2'
    ...
    allocate ( sx_x (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng) , &
               ...
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_xyz 3'
    ...
    allocate ( q_x (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng) , &
               ...
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_xyz 4'
    ...
    allocate ( wrk_x (sx:ex,sy:ey,sz:ez) , &
               ...
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_xyz 5'
    ...
    allocate ( wrka_x (sx:ex,sy:ey,sz:ez,nrv+npv) , &
               ...
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_xyz 6'
...
!! Benguerir, Morocco
  ...
  subroutine viscflux_eg_LES_xyz ( inp , thd , adi , dx_i , dy_i , dz_i , T , W_i , cp , ha , v , &
                                   rweg , iweg , fl )
    ...
    allocate ( rd          (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng,nrv,nrv) , &
               ...
               ct          (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)         , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_LES_xyz'
    ...
    allocate ( ux (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)         , &
               ...
               fd (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng,nderiv)  , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_LES_xyz 2'
    ...
    call dz_fixed1 ( dz_i , W_i  (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)      , &
                            fd (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng,nSGS+3) )
    ...
    allocate ( sx_x         (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng)      , &
               ...
               dtau_iso_SGS (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng,ndim) , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_LES_xyz 3'
       ...
       do k = k0 , k1
          do j = j0 , j1
             do i = i0 , i1
                ...
                sz_z (i,j,k) = bulk + twothirds * rmu * ( fd (i,j,k,9) + fd (i,j,k,9) - &
                                                        ( fd (i,j,k,1) + fd (i,j,k,5) ) )
                sz_z_SGS (i,j,k) = bulk + ( sz_z (i,j,k) - bulk ) * ( 1.0_dp + rmu_SGS ) + rtau_iso_SGS
          ...
          do j = j0 , j1
             do i = i0 , i1
                ...
                do l = 1 , nrv
                   ...
                   fd (i,j,k,nfluid+ndim*l-2) = - rvd0 * ( fd (i,j,k,nfluid+ndim*l-2) * ( 1.0_dp + wrk1 * W_i (i,j,k) ) + &
                                                           wrk1 * Xa (i,j,k,l) * fd (i,j,k,nSGS+1) )
                   ...
                   fd (i,j,k,nfluid+ndim*l  ) = - rvd0 * ( fd (i,j,k,nfluid+ndim*l  ) * ( 1.0_dp + wrk1 * W_i (i,j,k) ) + &
                                                           wrk1 * Xa (i,j,k,l) * fd (i,j,k,nSGS+3) )
    ...
    allocate ( q_x (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng) , &
               ...
               q_z (sx-ng:ex+ng,sy-ng:ey+ng,sz-ng:ez+ng) , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_LES_xyz 4'
    ...
    allocate ( wrk_x (sx:ex,sy:ey,sz:ez) , &
               ...
               wrk_z (sx:ex,sy:ey,sz:ez) , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_LES_xyz 5'
    ...
    allocate ( wrka_x (sx:ex,sy:ey,sz:ez,nrv+npv) , &
               ...
               wrka_z (sx:ex,sy:ey,sz:ez,nrv+npv) , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate viscflux_eg_LES_xyz 6'
