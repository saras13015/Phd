C
      SUBROUTINE CKINTP36(itoto,AWT,KNCF)
C      PROGRAM CKINTP
C
C----------------------------------------------------------------------C
C     VERSION 3.6
...
C                                            " =>" if irreversible.
C     2.  Fixed an error with IFIRCH36(LINE) in IPPLEN36
...
C     2.  Correct UPCASE36 to avoid dimensioning errors
...
C     1.  Error in UPCASE36 could cause interpreter to ignore some
C         keywords.
...
C
     ...
     2          UPCASE36*4, VERS*(LSYM), PREC*(LSYM)
C
...
C
      CALL CKISUB36 (LINE(:ILEN), SUB, NSUB)
C
C        IS THERE A KEYWORD?
C
      CALL CKCOMP36 ( UPCASE36(SUB(1), 4) , KEY, 5, NKEY)
...
C
         IF (NSUB .GT. 1) THEN
            IF ( UPCASE36(SUB(2), 3) .EQ. 'ALL') THEN
               THERMO = .FALSE.
               READ (LIN,'(A)') LINE
               CALL IPPARR36 (LINE, -1, 3, VALUE, NVAL, IER, LOUT)
...
C
         ...
         ENDIF
C
         CALL CKTHRM36 (LIN, MDIM, ENAME, MM, AWT, KNAME, KK, KNCF,
     1                KPHSE, KCHRG, WTM, MAXTP, NT, NTR, TLO, TMID,
     2                THI, T, NPCP2, A, ITHRM, KERR, LOUT, LINE)
C
         IF (.NOT. THERMO)
     1      CALL CKPRNT36(MDIM, MAXTP, MM, ENAME, KK, KNAME, WTM, KPHSE,
     2                    KCHRG, NT, T, TLO, TMID, THI, KNCF, ITHRM,
     3                    LOUT, KERR)
         I1 = IFIRCH36(LINE)
         IF (UPCASE36(LINE(I1:), 4) .EQ. 'REAC') GO TO 105
C
      ELSEIF (NKEY .EQ. 4) THEN
C
         ITASK = 4
C        START OF REACTIONS; ARE UNITS SPECIFIED?
         CALL CKUNIT36 (LINE(:ILEN), AUNITS, EUNITS, IUNITS)
C
         IF (THERMO) THEN
C
C           THERMODYNAMIC DATA
     ...
     1                   FILE='therm.dat')
            ...
            ELSE
               ...
               THI = VALUE(3)
            ENDIF
            CALL CKTHRM36 (LTHRM, MDIM, ENAME, MM, AWT, KNAME, KK, KNCF,
     1                     KPHSE, KCHRG, WTM, MAXTP, NT, NTR, TLO, TMID,
     2                     THI, T, NPCP2, A, ITHRM, KERR, LOUT, LINE)
            CALL CKPRNT36(MDIM, MAXTP, MM, ENAME, KK, KNAME, WTM, KPHSE,
...
C
         IF (NSUB .GT. 0) THEN
            M1 = MM +1
            CALL CKCHAR36 (SUB, NSUB, MDIM, ENAME, AWT, MM, KERR, LOUT)
            DO 110 M = M1, MM
               IF (AWT(M) .LE. 0) CALL CKAWTM36 (ENAME(M), AWT(M))
...
C
         ...
         IF (NSUB .GT. 0)
     1   CALL CKCHAR36 (SUB, NSUB, KDIM, KNAME, WTM, KK, KERR, LOUT)
C
...
C
         ...
         DO 120 N = 1, NSUB
            ...
            IF (UPCASE36(SUB(N), 3) .EQ. 'DUP') IND = MAX(IND,1)
  120    CONTINUE
         IF (IND .GT. 0) THEN
C
C           AUXILIARY REACTION DATA
C
            CALL CKAUXL36 (SUB, NSUB, II, KK, KNAME, LOUT, MAXSP, NPAR,
...
C
     1         CALL CPREAC36 (II, MAXSP, NSPEC, NPAR, PAR, RPAR,
...
C
               II = II+1
               CALL CKREAC36 (LINE(:ILEN), II, KK, KNAME, LOUT, MAXSP,
...
C
          CALL CPREAC36 (II, MAXSP, NSPEC, NPAR, PAR, RPAR, AUNITS,
...
C
  ...
  500    CONTINUE
C
         WRITE (LOUT, '(/1X,A)') ' NOTE: '//IUNITS(:ILASCH36(IUNITS))
C
      ELSEIF (THERMO) THEN
C
...
C**********************************************************************C
C----------------------------------------------------------------------C
C               SUBROUTINES AND FUNCTIONS USED IN CKINTP               C
C----------------------------------------------------------------------C
C**********************************************************************C

      SUBROUTINE CKCHAR36 (SUB, NSUB, NDIM, STRAY, RAY, NN, KERR, LOUT)
C
...
C
      DIMENSION RAY(*), PAR(1)
      CHARACTER SUB(*)*(*), STRAY(*)*(*), ISTR*80, UPCASE36*4
      LOGICAL KERR
C
      ILEN = LEN(STRAY(1))
C
      DO 200 N = 1, NSUB
         IF ( UPCASE36(SUB(N), 3) .EQ. 'END') RETURN
         ISTR = ' '
         I1 = INDEX(SUB(N),'/')
         IF (I1 .EQ .1) THEN
            KERR = .TRUE.
            WRITE (LOUT, 130) SUB(N)(:ILASCH36(SUB(N)))
         ELSE
            IF (I1 .LE. 0) THEN
               ISTR = SUB(N)
            ELSE
               ISTR = SUB(N)(:I1-1)
            ENDIF
            CALL CKCOMP36 (ISTR, STRAY, NN, INUM)
C
            IF (INUM .GT. 0) THEN
               WRITE (LOUT, 100) SUB(N)(:ILASCH36(SUB(N)))
            ELSE
               IF (NN .LT. NDIM) THEN
                  IF (ISTR(ILEN+1:) .NE. ' ') THEN
                     WRITE (LOUT, 120) SUB(N)(:ILASCH36(SUB(N)))
                     KERR = .TRUE.
                  ELSE
                     NN = NN + 1
                     STRAY(NN) = ' '
                     ...
                     IF (I1 .GT. 0) THEN
                        ...
                        CALL IPPARR36 (ISTR, 1, 1, PAR, NVAL, IER, LOUT)
                        IF (IER .EQ. 0) THEN
                           RAY(NN) = PAR(1)
                        ELSE
                           KERR = .TRUE.
                  ...
                  ENDIF
               ELSE
                  WRITE (LOUT, 110) SUB(N)(:ILASCH36(SUB(N)))
                  KERR = .TRUE.
               ENDIF
            ENDIF
         ENDIF
...
C
      ...
      CHARACTER ENAME*(*), IATOM(NATOM)*2, UPCASE36*2
C
...
C MATTHIEU
C     *'SC', 44.95600, 'TI', 47.90000, 'V ', 50.94200, 'CR', 51.99600,
     *'HM', 2.00797, 'OM',  4.00260, 'NM', 28.00670, 'H2OM', 18.0000,
C     *'MN', 54.93800, 'FE', 55.84700, 'CO', 58.93320, 'NI', 58.71000,
     *'HEM', 4.00260, 'FE', 55.84700, 'CO', 58.93320, 'NI', 58.71000,
...
C
      CALL CKCOMP36 ( UPCASE36(ENAME, 2), IATOM, NATOM, L)
      IF (L .GT. 0) AWT = ATOM(L)
      RETURN
      END
C----------------------------------------------------------------------C
      SUBROUTINE CKTHRM36 (LUNIT, MDIM, ENAME, MM, AWT, KNAME, KK, KNCF,
     1                   KPHSE, KCHRG, WTM, MAXTP, NT, NTR, TLO, TMID,
...
C
     ...
     1          KCHRG(*), A(NPCP2,NTR,*), AWT(*), VALUE(5)
      ...
      CHARACTER UPCASE36*4, ISTR*80, SUB(80)*80
      LOGICAL KERR, ITHRM(*)
C
...
C
      CALL CKISUB36 (ISTR(:ILEN), SUB, NSUB)
      CALL CKCOMP36 (SUB(1), KNAME, KK, K)
      IF (K .EQ. 0) THEN
         IF (UPCASE36(SUB(1), 3) .EQ. 'END' .OR.
     1       UPCASE36(SUB(1), 4) .EQ. 'REAC') RETURN
         GO TO 10
      ENDIF
C
      IF (ITHRM(K)) GO TO 10
...
C
         IF (ELEM.NE.' ' .AND. IELEM.NE.0) THEN
            IF (UPCASE36(ELEM, 1) .EQ. 'E')
     1             KCHRG(K)=KCHRG(K)+IELEM*(-1)
            CALL CKCOMP36 (ELEM, ENAME, MM, M)
            IF (M .GT. 0) THEN
               KNCF(M,K) = IELEM
   ...
   60 CONTINUE
C
      IF (UPCASE36(LINE(1)(45:),1) .EQ. 'L') KPHSE(K)=1
      IF (UPCASE36(LINE(1)(45:),1) .EQ. 'S') KPHSE(K)=-1
C
C-----Currently allows for three temperatures, two ranges;
C     in future, NT(K) may vary, NTR = NT(K)-1
C
      T(1,K) = TLO
      IF (LINE(1)(46:55) .NE. ' ') CALL IPPARR36
     1   (LINE(1)(46:55), 0, 1, T(1,K), NVAL, IER, LOUT)
C
      T(2,K) = TMID
      IF (LINE(1)(66:73) .NE. ' ') CALL IPPARR36
     1   (LINE(1)(66:73), 0, 1, T(2,K), NVAL, IER, LOUT)
C
      T(NT(K),K) = THI
      IF (LINE(1)(56:65) .NE. ' ') CALL IPPARR36
...
C
  ...
  100 FORMAT (6X,'Error...element...',A,'not declared for...',A)
      END
C----------------------------------------------------------------------C
      SUBROUTINE CKREAC36 (LINE,II,KK, KNAME, LOUT, MAXSP, NSPEC, NREAC,
...
C
     ...
     2          IRNU(*), RNU(MAXSP,*), IPLUS(20)
      CHARACTER KNAME(*)*(*), LINE*(*), CNUM(11)*1, UPCASE36*4
...
C
      CALL IPNPAR36 (LINE, NPAR, ISTR, ISTART)
      CALL IPPARR36 (ISTR, 1, NPAR, PAR(1,II), NVAL, IER, LOUT)
...
C
   ...
   25 CONTINUE
C
      IF (ILASCH36(INAME).GE.45 .AND. I1.GT.0) THEN
...
C
     ...
     1                   ISTR(I1:I2-1).EQ.'m') THEN
                         ...
                         ENDIF
                     ELSE
                        CALL CKCOMP36 (ISTR(I1:I2-1), KNAME, KK, KNUM)
                        IF (KNUM .GT. 0) THEN
                           IF (KTB .NE. 0) THEN
                              WRITE (LOUT, 630)
                              KERR = .TRUE.
...
C
     ...
     1               (NTHB.GT.0.AND.ITHB(NTHB).NE.II)) THEN
               ...
               ENDIF
C
            ELSEIF (UPCASE36(ISPEC, 2) .EQ. 'HV') THEN
               IF (LWL) THEN
                  WRITE (LOUT, 670)
                  KERR = .TRUE.
                  RETURN
...
C
            ...
            IF (IND .GT. 0) THEN
               IF (LRSTO) THEN
                  CALL IPPARR36 (ISPEC(:IND), 1, 1, RVAL, NVAL,
     1                         IER, LOUT)
               ELSE
                  CALL IPPARI36 (ISPEC(:IND), 1, 1, IVAL, NVAL,
     1                        IER, LOUT)
               ENDIF
               IF (IER .EQ. 0) THEN
                  ITEMP = ' '
            ...
            ENDIF
C
            CALL CKCOMP36 (ISPEC, KNAME, KK, KNUM)
            IF (KNUM .EQ. 0) THEN
               IF ((N-N1) .GT. 1) GO TO 510
               WRITE (LOUT, 680) ISPEC(:ILASCH36(ISPEC))
               KERR = .TRUE.
            ELSE
C
...
C 1900 FORMAT (I4,'. ',A,T51,E10.3,F7.3,F11.3)
 ...
 1920 FORMAT (6X,A)
      ...
      END
C----------------------------------------------------------------------C
      SUBROUTINE CKAUXL36 (SUB, NSUB, II, KK, KNAME, LOUT, MAXSP, NPAR,
     1                   NSPEC, NTHB, ITHB, NTBS, MAXTB, NKTB, AIK,
     2                   NFAL, IFAL, IDUP, NFAR, PFAL, IFOP, NLAN,
     3                   ILAN, NLAR, PLAN, NREV, IREV, RPAR, NRLT, IRLT,
     4                   RLAN, NWL, IWL, WL, KERR, NORD, IORD, MAXORD,
...
C
     ...
     5          NU(MAXSP,*), IRNU(*), RNU(MAXSP,*)
      CHARACTER SUB(*)*(*), KNAME(*)*(*), KEY*80, RSTR*80, UPCASE36*4,
     1          ISTR*80
...
C
      DO 500 N = 1, NSUB
         ...
         KEY = ' '
C
         IF ( UPCASE36(SUB(N), 3) .EQ. 'DUP') THEN
            IDUP(II) = -1
            WRITE (LOUT, 4000)
            GO TO 500
         ELSE
         ...
         ENDIF
C
         IF (UPCASE36(KEY, 3).EQ.'LOW' .OR.
     1       UPCASE36(KEY, 4).EQ.'TROE'.OR.
     2       UPCASE36(KEY, 3).EQ.'SRI') THEN
C
C        FALL-OFF DATA
C
            ...
            ELSE
C
               IF (UPCASE36(KEY, 3) .EQ. 'LOW') THEN
                  IF (IFOP(NFAL) .GT. 0) THEN
                     WRITE (LOUT, 2000) SUB(N)(:ILEN)
                     KERR = .TRUE.
                  ELSE
                     IFOP(NFAL) = ABS(IFOP(NFAL))
                     CALL IPPARR36 (RSTR,1,3,PFAL(1,NFAL),NVAL,IER,LOUT)
                     IF (IER .NE. 0) KERR = .TRUE.
                     WRITE (LOUT, 3050) (PFAL(L,NFAL),L=1,3)
                  ENDIF
C
               ELSEIF (UPCASE36(KEY, 4) .EQ. 'TROE') THEN
                  ...
                  ELSE
                     LTRO = .TRUE.
                    CALL IPPARR36 (RSTR,1,-4,PFAL(4,NFAL),NVAL,IER,LOUT)
                     IF (NVAL .EQ. 3) THEN
                  ...
                  ENDIF
C
               ELSEIF (UPCASE36(KEY, 3) .EQ. 'SRI') THEN
                  IF (LTRO .OR. LSRI) THEN
                     KERR = .TRUE.
                  ...
                  ELSE
                    ...
                    CALL IPPARR36 (RSTR,1,-5,PFAL(4,NFAL),NVAL,IER,LOUT)
                     IF (NVAL .EQ. 3) THEN
                        PFAL(7,NFAL) = 1.0
                        PFAL(8,NFAL) = 0.0
            ...
            ENDIF
C
         ELSEIF (UPCASE36(KEY, 3) .EQ. 'REV') THEN
C
...
C
            ...
            ELSE
               ...
               NREV = NREV+1
               IREV(NREV) = II
               CALL IPPARR36 (RSTR,1,NPAR,RPAR(1,NREV),NVAL,IER,LOUT)
               IF (IER .NE. 0) KERR = .TRUE.
               WRITE (LOUT, 1900) '   Reverse Arrhenius coefficients:',
     1                           (RPAR(L,NREV),L=1,3)
            ENDIF
C
         ELSEIF (UPCASE36(KEY, 3) .EQ. 'RLT') THEN
C
...
C
            ...
            ELSE
               ...
               NRLT = NRLT + 1
               IRLT(NRLT) = II
               CALL IPPARR36 (RSTR,1,NLAR,RLAN(1,NRLT),NVAL,IER,LOUT)
               IF (IER .NE. 0) KERR = .TRUE.
               WRITE (LOUT, 3040) (RLAN(L,NRLT),L=1,2)
            ENDIF
C
         ELSEIF (UPCASE36(KEY, 2) .EQ. 'HV') THEN
C
...
C
            IF (.NOT.LWL) THEN
               ...
               KERR = .TRUE.
            ELSE
               CALL IPPARR36 (RSTR,1,1,VAL,NVAL,IER,LOUT)
               IF (IER .EQ. 0) THEN
                  WL(NWL) = WL(NWL)*VAL(1)
            ...
            ENDIF
C
         ELSEIF (UPCASE36(KEY, 2) .EQ. 'LT') THEN
C
...
C
            ...
            ELSE
               ...
               NLAN = NLAN + 1
               ILAN(NLAN) = II
               CALL IPPARR36 (RSTR,1,NLAR,PLAN(1,NLAN),NVAL,IER,LOUT)
               IF (IER .NE. 0) THEN
            ...
            ENDIF
C
         ELSEIF (UPCASE36(KEY,4).EQ.'FORD' .OR.
     1           UPCASE36(KEY,4).EQ.'RORD') THEN
             LFORD = (UPCASE36(KEY,4) .EQ. 'FORD')
             LRORD = (UPCASE36(KEY,4) .EQ. 'RORD')
...
C
  ...
  113              CONTINUE
             ...
             ENDIF
C
             CALL IPNPAR36 (RSTR, 1, ISTR, ISTART)
             IF (ISTART .GE. 1) THEN
                CALL IPPARR36 (ISTR, 1, 1, VAL, NVAL, IER, LOUT)
                CALL CKCOMP36 (RSTR(:ISTART-1), KNAME, KK, K)
                IF (LFORD) K = -K
                NK = 0
                DO 121 L = 1, MAXORD
C
...
C
            CALL CKCOMP36 (KEY, KNAME, KK, K)
            IF (K .EQ. 0) THEN
               WRITE (LOUT, 1040) KEY(:ILASCH36(KEY))
               KERR = .TRUE.
            ELSE
               IF (.NOT.LTHB) THEN
                  KERR = .TRUE.
...
C 1900 FORMAT (6X,A,T51,E10.3,F7.3,F11.3)
 ...
 4000 FORMAT (6X,'Declared duplicate reaction...')
      END
C----------------------------------------------------------------------C
      SUBROUTINE CKPRNT36 (MDIM, MAXTP, MM, ENAME, KK, KNAME, WTM,
...
C
         CALL CKCOMP36 (KNAME(K)(:1), INUM, 10, I)
         IF (I .GT. 0) THEN
            KERR = .TRUE.
            WRITE (LOUT, 210)
         ENDIF
...
C
               IF (KNAME(K)(I1-1:I1-1) .EQ. '(') THEN
                  ...
                  IF (I2 .GT. I1) THEN
                     CALL CKCOMP36 (KNAME(K)(I1:I2-1), KNAME, KK, KNUM)
                     IF (KNUM .GT. 0) THEN
                        WRITE (LOUT, 230)
                        KERR = .TRUE.
                     ENDIF
...
C
               I2 = ILASCH36(KNAME(K))
               IF (I2 .GE. I1) THEN
                  CALL CKCOMP36 (KNAME(K)(I1:I2), KNAME, KK, KNUM)
                  IF (KNUM .GT. 0) THEN
                     WRITE (LOUT, 230)
                     KERR = .TRUE.
                  ENDIF
...
C
  ...
  500 FORMAT (1X,I3,'. ',A16,T24,A1,T26,I1,T28,F9.5,T38,F6.1,T45,F6.1,
     1       T51,15(I3))
      END
C----------------------------------------------------------------------C
      SUBROUTINE CPREAC36 (II, MAXSP, NSPEC, NPAR, PAR, RPAR, AUNITS,
...
C
      IF (NRNU.GT.0 .AND. (II.EQ.IRNU(NRNU))) THEN
         CALL CKRBAL36 (MAXSP, NUNK(1,II), RNU(1,NRNU), MDIM, MM, KCHRG,
     1                KNCF, CKMIN, IERR)
      ELSE
         CALL CKBAL36 (MAXSP, NUNK(1,II),NU(1,II),MDIM, MM, KCHRG, KNCF,
     1               IERR)
      ENDIF
C
      IF (IERR) THEN
      ...
      ENDIF
C
      CALL CKDUP36 (II, MAXSP, NSPEC, NREAC, NU, NUNK, NFAL, IFAL, KFAL,
     1            ISAME)
C
      IF (ISAME .GT. 0) THEN
...
C
 ...
 1060 FORMAT (6X,'Error...reaction does not balance...')
      ...
      END
C----------------------------------------------------------------------C
      SUBROUTINE CKBAL36 (MXSPEC, KSPEC, KCOEF, MDIM, MM, KCHRG, KNCF,
     1                  IERR)
C
...
C
  ...
  100 CONTINUE
      ...
      END
C----------------------------------------------------------------------C
      SUBROUTINE CKRBAL36 (MXSPEC, KSPEC, RCOEF, MDIM, MM, KCHRG, KNCF,
     1                   CKMIN, IERR)
C
C     Checks elemental balance of reactants vs. products.
C     Checks charge balance of reaction.
...
C
  ...
  100 CONTINUE
      ...
      END
C----------------------------------------------------------------------C
      SUBROUTINE CKDUP36 (I, MAXSP, NS, NR, NU, NUNK, NFAL, IFAL, KFAL,
     1                  ISAME)
C
...
C
  500 CONTINUE
      ...
      END
C----------------------------------------------------------------------C
      SUBROUTINE CKISUB36 (LINE, SUB, NSUB)
C
...
C
    ...
    5 CONTINUE
C
      IF (IPPLEN36(LINE) .LE. 0) RETURN
C
      ILEN = ILASCH36(LINE)
C
      NSTART = IFIRCH36(LINE)
   10 CONTINUE
      ISTART = NSTART
      NSUB = NSUB + 1
      SUB(NSUB) = ' '
...
C
         NSTART = ILAST + IFIRCH36(LINE(ILAST+1:))
C
...
C
         ...
         IF (I1 .LE. 0) THEN
            ...
            NSTART = NEND + IFIRCH36(LINE(NEND+1:))
            GO TO 10
         ENDIF
C
...
C
  ...
  100 CONTINUE
      ...
      END
C----------------------------------------------------------------------C
      SUBROUTINE IPNPAR36 (LINE, NPAR, IPAR, ISTART)
C
...
C
   ...
   40    CONTINUE
      ...
      END
C----------------------------------------------------------------------C
      SUBROUTINE IPPARI36 (STRING,ICARD,NEXPEC,IVAL, NFOUND, IERR, LOUT)
...
C
C   REFERENCES  (NONE)
C   ROUTINES CALLED  IFIRCH36,ILASCH36
...
C
     ...
     1   ' TOO MANY DATA ITEMS.  NUMBER EXPECTED = ', NEXPEC
      END
C
      SUBROUTINE IPPARR36(STRING,ICARD,NEXPEC, RVAL, NFOUND, IERR, LOUT)
...
C
     ...
     1   ' TOO MANY DATA ITEMS.  NUMBER EXPECTED = ', NEXPEC
      END
C
      FUNCTION IFIRCH36(STRING)
C   BEGIN PROLOGUE  IFIRCH36
...
C
C-----------------------------------------------------------------------
C  IFIRCH36 locates the first non-blank character in a string of
C  arbitrary length.  If no characters are found, IFIRCH36 is set = 0.
C  When used with the companion routine ILASCH36, the length of a string
...
C
      CHARACTER* (*)STRING
C
C   FIRST EXECUTABLE STATEMENT IFIRCH36
      NLOOP = LEN(STRING)
C
      IF (NLOOP .EQ. 0) THEN
         IFIRCH36 = 0
...
120   CONTINUE
      IFIRCH36 = I
      END
      FUNCTION ILASCH36(STRING)
C   BEGIN PROLOGUE  ILASCH36
C   DATE WRITTEN   850626
C   REVISION DATE  850626
C   CATEGORY NO.  M4.
C   KEYWORDS  CHARACTER STRINGS,SIGNIFICANT CHARACTERS
...
C
C-----------------------------------------------------------------------
C  IFIRCH36 locates the last non-blank character in a string of
C  arbitrary length.  If no characters are found, ILASCH36 is set = 0.
C  When used with the companion routine IFIRCH36, the length of a string
C  can be determined, and/or a concatenated substring containing the
C  significant characters produced.
...
C
      CHARACTER*(*) STRING
C
C***FIRST EXECUTABLE STATEMENT ILASCH36
      NLOOP = LEN(STRING)
      IF (NLOOP.EQ.0) THEN
         ILASCH36 = 0
         RETURN
...
C----------------------------------------------------------------------C
C
      SUBROUTINE CKCOMP36 (IST, IRAY, II, I)
C
C  START PROLOGUE
C
...
C
C----------------------------------------------------------------------C
      SUBROUTINE CKUNIT36 (LINE, AUNITS, EUNITS, IUNITS)
C
C*****precision > double
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER (I-N)
C*****END precision > double
...
C*****END precision > single
      ...
      CHARACTER*4 UPCASE36
C
      AUNITS = ' '
      EUNITS = ' '
      IUNITS = ' '
      LCHAR = ILASCH36(LINE)
      DO 85 N = 1, ILASCH36(LINE)-3
         IND = ILASCH36(IUNITS)
         IF (EUNITS .EQ. ' ') THEN
            IF (UPCASE36(LINE(N:), 4)     .EQ. 'CAL/') THEN
               EUNITS = 'CAL/'
               IF (IUNITS .EQ. ' ') THEN
               ...
               ENDIF
            ELSEIF (UPCASE36(LINE(N:), 4) .EQ. 'KCAL') THEN
               EUNITS = 'KCAL'
               IF (IUNITS .EQ. ' ') THEN
               ...
               ENDIF
            ELSEIF (UPCASE36(LINE(N:), 4) .EQ. 'JOUL') THEN
               EUNITS = 'JOUL'
               IF (IUNITS .EQ. ' ') THEN
               ...
               ENDIF
            ELSEIF (UPCASE36(LINE(N:), 4) .EQ. 'KJOU') THEN
               EUNITS = 'KJOU'
               IF (IUNITS .EQ. ' ') THEN
               ...
               ENDIF
            ELSEIF (UPCASE36(LINE(N:), 4) .EQ. 'KELV') THEN
               EUNITS = 'KELV'
               IF (IUNITS .EQ. ' ') THEN
         ...
         ENDIF
         IF (AUNITS .EQ. ' ') THEN
            IF (UPCASE36(LINE(N:), 4) .EQ. 'MOLE') THEN
               IF (N+4.LE.ILASCH36(LINE) .AND.
     1                    UPCASE36(LINE(N+4:),1).EQ.'C') THEN
C
                  AUNITS = 'MOLC'
                  IF (IUNITS .EQ. ' ') THEN
...
C----------------------------------------------------------------------C
C
      INTEGER FUNCTION IPPLEN36 (LINE)
C
C  BEGIN PROLOGUE
C
C  FUNCTION IPPLEN36 (LINE)
...
C
C  OUTPUT
C     IPPLEN36 - The effective length of the character string.
C
C  END PROLOGUE
C
C*****precision > double
...
C
      ...
      ELSE
         IN = INDEX(LINE,'!')
         IF (IN .EQ. 0) THEN
            IPPLEN36 = ILASCH36(LINE)
         ELSE
            IPPLEN36 = ILASCH36(LINE(:IN-1))
         ENDIF
      ENDIF
      RETURN
      END
C
      CHARACTER*(*) FUNCTION UPCASE36(ISTR, ILEN)
...
C
      UPCASE36 = ' '
      UPCASE36 = ISTR(:ILEN)
      JJ = MIN (LEN(UPCASE36), LEN(ISTR), ILEN)
      DO 10 J = 1, JJ
         DO 10 N = 1,26
            IF (ISTR(J:J) .EQ. LCASE(N)) UPCASE36(J:J) = UCASE(N)
   10 CONTINUE
      RETURN
      END
