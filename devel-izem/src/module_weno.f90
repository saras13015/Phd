...
module weno
  ...
  use type_thd
  use adim
  use thermodynamics , only : Wmix_i_scalar , cp_scalar , ha_scalar
  ...
  real (dp) , private :: za , zb , zc , zd , ze , zf , zg , zh , &
                         ...
                         zs , zt , zu
  real (dp) , private :: z1, z2
  !real (dp) , private :: cnr ( 2*ng-1 )

  ! Defining cweno, dweno, dtweno & cnr dynamically (ng is not a parameter anymore)

  real(dp), allocatable, dimension(:, :), private :: cweno
  real(dp), allocatable, dimension(:), private ::  dweno, dtweno, cnr
...
!!
!! Definition of matrix c(r,j) of Shu (page 8) end of the coefficients
!! for the smoothness indicators
!!
!!
!> \author
!! Modeling, Simulation and Data Analysis (MSDA)\n
...
!! Benguerir, Morocco
  ...
  allocate ( cweno ( -1:ng-1 , 0:ng-1 ) , &
    ...
    else if (ng == 3) then ! coefficients for 5-th order

       cweno (-1,0)     =   11.0_dp / 6.0_dp
       cweno (-1,1)     = - 7.0_dp / 6.0_dp
       cweno (-1,2)     =   1.0_dp / 3.0_dp

       cweno (0,0)      =   1.0_dp / 3.0_dp
       cweno (0,1)      =   5.0_dp / 6.0_dp
       cweno (0,2)      = - 1.0_dp / 6.0_dp

       cweno (1,0)      = - 1.0_dp / 6.0_dp
       cweno (1,1)      =   5.0_dp / 6.0_dp
       cweno (1,2)      =   1.0_dp / 3.0_dp

       cweno (2,0)      =   1.0_dp / 3.0_dp
       cweno (2,1)      = - 7.0_dp / 6.0_dp
       cweno (2,2)      =   11.0_dp / 6.0_dp 

       ! (r=0,1,2)
       dweno (0)   =   3.0_dp / 10.0_dp
       dweno (1)   =   3.0_dp / 5.0_dp
       dweno (2)   =   1.0_dp / 10.0_dp
       !??? dtweno
       dtweno (0)   =   dweno (2)
       dtweno (1)   =   dweno (1)
       dtweno (2)   =   dweno (0)
...
!    real (dp) , parameter :: denom = 1.0_dp / 420.0_dp
  ...
  if (ng == 4) then
    ...
    denom = 1.0_dp / 60.0_dp
          
    vminus = ( - 3.0_dp * vp(1) + 27.0_dp * vp(2) + 47.0_dp * vp(3)  &
               - 13.0_dp * vp(4) + 2.0_dp * vp(5)  ) * denom

    vplus  = ( - 3.0_dp * vm(6) + 27.0_dp * vm(5) + 47.0_dp * vm(4)  &
               - 13.0_dp * vm(3) + 2.0_dp * vm(2)  ) * denom
...
!! Benguerir, Morocco
  ...
  if (ng == 4) then
    ...
    beta (0) = z1 * ( vp(i)    + za*vp(i+1) + vp(i+2) )  * &
                    ( vp(i)    + za*vp(i+1) + vp(i+2) )      + &
               z2 * ( zc*vp(i) + zb*vp(i+1) + vp(i+2) )  * &
                    ( zc*vp(i) + zb*vp(i+1) + vp(i+2) ) 

    beta (1) = z1 * ( vp(i-1)  + za*vp(i)   + vp(i+1) )  * &
                    ( vp(i-1)  + za*vp(i)   + vp(i+1) )      + &
               z2 * ( vp(i-1)  + zd*vp(i+1) )            * &
                    ( vp(i-1)  + zd*vp(i+1) ) 

    beta (2) = z1 * ( vp(i-2)  + za*vp(i-1) + vp(i) )    * &
                    ( vp(i-2)  + za*vp(i-1) + vp(i) )        + &
               z2 * ( vp(i-2)  + zb*vp(i-1) + zc*vp(i) ) * &
                    ( vp(i-2)  + zb*vp(i-1) + zc*vp(i) ) 

!    beta (0) = vp(i)   * ( ze*vp(i) + zf*vp(i+1) + zc*vp(i+2) ) + &
!               vp(i+1) * (            zd*vp(i+1) + zb*vp(i+2) ) + &
!               vp(i+2) * (                         za*vp(i+2) )
!
!    beta (1) = vp(i-1) * ( za*vp(i-1) + zg*vp(i) + zh*vp(i+1) ) + &
!               vp(i)   * (            zi*vp(i)   + zg*vp(i+1) ) + &
!               vp(i+1) * (                         za*vp(i+1) )
!
!    beta (3) = vp(i-2) * ( za*vp(i-2) + zb*vp(i-1) + zc*vp(i) )  + &
!               vp(i-1) * (              zd*vp(i-1) + zf*vp(i) )  + &
!               vp(i)   * (                           ze*vp(i) )  
!
  else
 ...
 !   do r = 0 , 3
  ...
  if (ng == 4) then
    ...
    beta (0) = z1 * ( vm(i)    + za*vm(i+1) + vm(i+2) )  * &
                    ( vm(i)    + za*vm(i+1) + vm(i+2) )       + &
               z2 * ( zc*vm(i) + zb*vm(i+1) + vm(i+2) )  * &
                    ( zc*vm(i) + zb*vm(i+1) + vm(i+2) ) 

    beta (1) = z1 * ( vm(i-1)  + za*vm(i)   + vm(i+1) )  * &
                    ( vm(i-1)  + za*vm(i)   + vm(i+1) )       + &
               z2 * ( vm(i-1)  + zd*vm(i+1) )            * &
                    ( vm(i-1)  + zd*vm(i+1) ) 

    beta (2) = z1 * ( vm(i-2)  + za*vm(i-1) + vm(i) )    * &
                    ( vm(i-2)  + za*vm(i-1) + vm(i) )         + &
               z2 * ( vm(i-2)  + zb*vm(i-1) + zc*vm(i) ) * &
                    ( vm(i-2)  + zb*vm(i-1) + zc*vm(i) ) 
...
!! Benguerir, Morocco
  ...
  subroutine euler_LLF_x ( thd , adi , dx_i , x , y , z , T , W_i , cp , ha , shock_sensor , v , fl )
   ...
   ! real (dp) , dimension (stencil_m1)        :: gplus , gminus , wc , gc
    real (dp) , allocatable,dimension (:,:)     :: f_s , L_s

    real (dp) , allocatable, dimension (:)        :: gplus , gminus , wc , gc
    ...
    allocate ( fhat  (sx-1:ex,nv)            , &
               ...
               gc    (stencil_m1)            , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate euler_LLF_x'
    ...
    do k = sz , ez ! loop in the z-direction
       do j = sy , ey ! loop in the y-direction
          ...
          do i_c = sx-ng , ex+ng
             ...
             gam (i_c) = thd % gam2 * W_i (i_c,j,k) / cp (i_c,j,k)
...
!              if ( j   > nty-ng ) shk = .true.
             ...
             do m = 1 , nv ! loop on the five char. fields
                ...
                if (any(shock_sensor(i_l-ng+1:i_l+ng,j,k))  .and. weno_avg ) then
    ...
    deallocate (f_s, L_s, gplus, gminus, wc, gc)
...
!! Benguerir, Morocco
  ...
  subroutine euler_LLF_y ( thd , adi , dy_i , x , y , z , T , W_i , cp , ha , shock_sensor , v , fl )
...
!    integer (ip) , parameter                  :: stencil_m1 = ng+ng
    ...
    !real (dp) , dimension (stencil_m1)        :: gplus , gminus , wc , gc
    ! We define them dynamically because ng is not a parameter anymore
    real (dp) , allocatable, dimension (:,:)     :: f_s , L_s
    ...
    allocate ( fhat  (sy-1:ey,nv)            , &
               ...
               gc    (stencil_m1)            , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate euler_LLF_y'
    ...
    do k = sz , ez ! loop in the z-direction
       do i = sx , ex ! loop in the x-direction
          ...
          do j_c = sy-ng , ey+ng
             ...
             P (j_c)     = v (i,j_c,k,1) * T (i,j_c,k) * W_i (i,j_c,k)
             ...
             gam (j_c) = thd % gam2 * W_i (i,j_c,k) / cp (i,j_c,k)
             gam (j_c) = 1.0_dp / ( 1.0_dp - gam (j_c) )
             cs (j_c)  = sqrt ( gam (j_c) * P (j_c) * rho_i (j_c) )
          ...
          do j_c = sy-1 , ey+1
             ...
             do l = 1 , nrv+npv+nvv
                Ya (j_c,l) = v (i,j_c,k,niv+l) * rho_i (j_c)
             end do
             ht (j_c) = 0.0_dp
             do l = 1 , nrv
                ht (j_c) = ht (j_c) + ha (i,j_c,k,l) * Ya (j_c,l)
             end do
             ht (j_c) = ht (j_c) + 0.5_dp * ( ux(j_c)*ux(j_c) + vy(j_c)*vy(j_c) + wz(j_c)*wz(j_c) )
...
!              if ( j_l > nty-ng ) shk = .true.
             ...
             do m = 1 , nv ! loop on the five char. fields
                ...
                ! Reconstruction of the '+' and '-' fluxes (page 32)
                if (any(shock_sensor(i,j_l-ng+1:j_l+ng,k))  .and. weno_avg ) then
                   call wenorec    ( gplus , gminus , gl , gr )
                else
                   call wenorec_nw ( gplus , gminus , gl , gr )
                end if
          ...
          do m = 1 , nv
             do j_c = sy , sy
                j_l = j_c - 1
                df = ( fhat(j_c,m) - fhat(j_l,m) ) * dy_i (j_c)
...
!! Benguerir, Morocco
  ...
  subroutine euler_LLF_z ( thd , adi , dz_i , x , y , z , T , W_i , cp , ha , shock_sensor , v , fl )
   ...
   ! We define them dynamically because ng is not a parameter anymore
    ...
    allocate ( fhat  (sz-1:ez,nv)            , &
               ...
               gc    (stencil_m1)            , &
               stat = ok )
    if ( ok > 0 ) stop 'error allocate euler_LLF_z'
    ...
    do j = sy , ey ! loop in the y-direction
       do i = sx , ex ! loop in the x-direction
          ...
          do k_c = sz-ng , ez+ng
             ...
             P (k_c)     = v (i,j,k_c,1) * T (i,j,k_c) * W_i (i,j,k_c)
             ...
             gam (k_c) = thd % gam2 * W_i (i,j,k_c) / cp (i,j,k_c)
             gam (k_c) = 1.0_dp / ( 1.0_dp - gam (k_c) )
             cs (k_c)  = sqrt ( gam (k_c) * P (k_c) * rho_i (k_c) )
          ...
          do k_c = sz-1 , ez+1
             ...
             do l = 1 , nrv+npv+nvv
                Ya (k_c,l) = v (i,j,k_c,niv+l) * rho_i (k_c)
             end do
             ht (k_c) = 0.0_dp
             do l = 1 , nrv
                ht (k_c) = ht (k_c) + ha (i,j,k_c,l) * Ya (k_c,l)
             end do
             ht (k_c) = ht (k_c) + 0.5_dp * ( ux(k_c)*ux(k_c) + vy(k_c)*vy(k_c) + wz(k_c)*wz(k_c) )
...
!              if ( k_l > ez-ng .and. bc (F) == noreflection ) shk = .true.
             ...
             do m = 1 , nv ! loop on the five char. fields
                ...
                ! Reconstruction of the '+' and '-' fluxes (page 32)
                if (any(shock_sensor(i,j,k_l-ng+1:k_l+ng))  .and. weno_avg ) then
                   call wenorec    ( gplus , gminus , gl , gr )
                else
                   call wenorec_nw ( gplus , gminus , gl , gr )
                end if
          ...
          do m = 1 , nv
             do k_c = sz , sz
                k_l = k_c - 1
                df = ( fhat(k_c,m) - fhat(k_l,m) ) * dz_i (k_c)
...
!! Benguerir, Morocco
  ...
  subroutine bc_noreflectionweno ( face , thd , dx_i , dy_i , dz_i , T , W_i , cp , ha , v , fl )
    ...
    integer (ip)                      :: stencil_m1, stencil_m2
    ...
    !Definition of stencil_m1 and stencil_m2
    stencil_m1 = ng+ng 
    stencil_m2 = ng+ng-1
...
!          read (*,*)
             ...
             do m = 1 , nv
                ...
                fl (i_c,j,k,m) = fl (i_c,j,k,m) + df * dx_i (i_c)
             end do
...
!          read (*,*)
             ...
             do m = 1 , nv
                ...
                fl (i,j_c,k,m) = fl (i,j_c,k,m) + df * dy_i (j_c)
             end do
...
!          read (*,*)
             ...
             do m = 1 , nv
                ...
                fl (i,j,k_c,m) = fl (i,j,k_c,m) + df * dz_i (k_c)
             end do
