...
module input
   ...
   subroutine parsing_input (inp , pars)
      ...
      call CFG_add (pars , "WENO%ORDER_WENO" , 7 , "ORDER OF ACCURACY WENO SCHEME (VALUE)")
      ...
      call CFG_get (pars , "WENO%ORDER_WENO", inp % order_weno)
...
subroutine assessing_input (inp)
   ...
   ! Check value of order weno scheme
   if ( .NOT.(order_weno == 5 .OR. order_weno == 7) ) then
        write (*,*) ' error: only 5th or 7th order weno scheme are considered'
        write (*,*) ' please choose between these two order values'
        call mpi_abort ( MPI_COMM_WORLD , 1 , mpicode )
   endif
   ! Initializing ng: nb of ghost points
   ng = (order_weno + 1) / 2 !! Attention aux arrondis!
