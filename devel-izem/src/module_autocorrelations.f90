...
!! Benguerir, Morocco
  ...
  subroutine correlation_Z ( inp , xt , yt , zt , vv , corr )
...
!! Benguerir, Morocco
  ...
  subroutine last_correlation_Z ( inp , xt , yt , zt , corr )
    ...
    do pz = 1 , dims (1)
       ...
       do py = 1 , dims (2)
          ...
          do px = 1 , dims (3)
             ...
             if ( pt (1) >= xt (ix) .and. pt (1) <= xt (fx) .and. &
                  pt (2) >= yt (iy) .and. pt (2) <= yt (fy) .and. &
                  pt (3) >= zt (iz) .and. pt (3) <= zt (fz) ) then
                myrank = np
             end if
...
!! Benguerir, Morocco
  ...
  subroutine integral_length_Z ( inp , xt , yt , zt , dz_i , corr , length )
    ...
    n_z2 = ntz/2-1
    do k = sz,min(ez,pz+n_z2)
       if ( k >= pz ) tmp1 = tmp1 + corr (sx,sy,k) / dz_i (k) ! sx and sy index are valid
    end do
